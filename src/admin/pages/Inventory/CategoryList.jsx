...
import { MagnifyingGlassIcon, PlusIcon, XCircleIcon, ChevronDownIcon, PencilIcon, ArrowRightOnRectangleIcon, TrashIcon } from '@heroicons/react/24/solid';
import { getInventoryCategories, createInventoryCategory, updateInventoryCategory, deleteInventoryCategory } from '../../../api/Inventory/inventoryCategory';
...
const CategoryList = () => {
  ...
  const fetchData = useCallback(async (goToPage) => {
    ...
    setError('');
    try {
      const res = await getInventoryCategories({ pageLimit: PAGE_LIMIT, pageNumber: goToPage, search });
  ...
  const handleAddCategory = async (e) => {
    ...
    setFormError('');
    try {
      await createInventoryCategory({ icName: newCategoryName, icType: newCategoryType });
  ...
  const handleUpdateCategory = async (e) => {
    ...
    setFormError('');
    try {
      await updateInventoryCategory(editingCategory.icId, { icName: editedCategoryName, icType: editedCategoryType });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const currentPageData = categories.filter(cat => cat.icId !== deletingCategory.icId);
    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="max-w-5xl mx-auto py-10 px-4 font-montserrat">
          <h1 className="text-4xl font-bold text-center text-primaryColor mb-2">DAFTAR KATEGORI</h1>
          <p className="text-center text-gray-500 mb-8">Berikut adalah daftar kategori yang terdaftar dalam sistem.</p>
            ...
            {loading ? (
              ...
              <table className="w-full text-left border-separate border-spacing-y-2">
                ...
                <tbody>
                  {categories.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="text-center py-6 text-gray-400">Tidak ada data kategori</td>
                    </tr>
                  ) : (
                    categories.map((cat) => {
                      return (
                        <tr key={cat.icId} className="bg-gray-100 hover:bg-secondaryColor/10 rounded-lg shadow-sm">
                          <td className="px-4 py-3 font-medium">{cat.icName}</td>
                          <td className="px-4 py-3">{categoryTypeMap[cat.icType]}</td>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
              <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
                ...
                <form onSubmit={handleAddCategory}>
                  <div className='mb-4'>
                    <label htmlFor="categoryName" className="block text-sm font-medium text-gray-700 mb-1">Nama Kategori</label>
                    <input
                      id="categoryName"
                      type="text"
                  ...
                  <div className='mb-4'>
                    <label htmlFor="categoryType" className="block text-sm font-medium text-gray-700 mb-1">Tipe Kategori</label>
                    <select
                      id="categoryType"
                      value={newCategoryType}
                      onChange={(e) => setNewCategoryType(Number(e.target.value))}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
              <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
                ...
                <form onSubmit={handleUpdateCategory}>
                  <div className='mb-4'>
                    <label htmlFor="editCategoryName" className="block text-sm font-medium text-gray-700 mb-1">Nama Kategori</label>
                    <input
                      id="editCategoryName"
                      type="text"
                  ...
                  <div className='mb-4'>
                    <label htmlFor="editCategoryType" className="block text-sm font-medium text-gray-700 mb-1">Tipe Kategori</label>
                    <select
                      id="editCategoryType"
                      value={editedCategoryType}
                      onChange={(e) => setEditedCategoryType(Number(e.target.value))}
                      className="w-full px-4 py-2 border border-gray-300 rounded bg-white focus:outline-none focus:ring-2 focus:ring-secondaryColor"
                    >
                      <option value={1}>Bahan Baku</option>
                      <option value={0}>Aksesoris</option>
