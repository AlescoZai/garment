...
import { ArrowLeftIcon, PlusIcon, TrashIcon, UserIcon, CalendarDaysIcon, PhoneIcon, MapPinIcon, DocumentTextIcon, PhotoIcon, ExclamationCircleIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const AddOrder = () => {
  ...
  // Helper functions untuk ID dan Name
  const getId = (item, idFields = ['cpId', 'iId', 'isId', 'sId', 'id']) => {
  ...
  const removeSizeFromItem = (itemIndex, sizeIndex) => {
    setFormData(prev => {
      ...
      if (newItems[itemIndex].oiSizes.length > 1) {
        newItems[itemIndex] = {
          ...newItems[itemIndex],
          oiSizes: newItems[itemIndex].oiSizes.filter((_, index) => index !== sizeIndex)
        };
      }
      return {
        ...prev,
  ...
  const handleMockupImageChange = (itemIndex, files) => {
    ...
    if (validFiles.length !== imageFiles.length) {
      toast.error('Beberapa file tidak valid. Hanya file JPG, PNG, WebP dengan ukuran maksimal 5MB yang diperbolehkan.');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.oItems.length === 0) {
      ...
      formData.oItems.forEach((item, index) => {
        if (!item.cpId || item.cpId === 0) {
          validationErrors.push(`Item ${index + 1}: Produk katalog wajib dipilih`);
        }
        if (!item.isId || item.isId === 0) {
          validationErrors.push(`Item ${index + 1}: Inventory stock wajib dipilih`);
        }
        if (!item.oiSizes || item.oiSizes.length === 0) {
          validationErrors.push(`Item ${index + 1}: Minimal satu ukuran harus dipilih`);
        } else {
          item.oiSizes.forEach((size, sizeIndex) => {
            if (!size.sId || size.sId === 0) {
              validationErrors.push(`Item ${index + 1} Ukuran ${sizeIndex + 1}: Ukuran wajib dipilih`);
            }
            if (!size.oisAmount || size.oisAmount <= 0) {
              validationErrors.push(`Item ${index + 1} Ukuran ${sizeIndex + 1}: Jumlah harus lebih dari 0`);
            }
          });
        }
      });
    ...
    try {
      ...
      setUploading(false);
      toast.error('Gagal membuat pesanan: ' + (err.response?.data?.message || err.message));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex-1">
        ...
        <div className="w-full max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 font-montserrat">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold text-primaryColor">Tambah Pesanan Baru</h1>
              <p className="text-gray-600">Buat pesanan baru dengan detail lengkap</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Nomor PO</label>
                  <input
                    type="text"
                    value={formData.oPoNumber}
                    onChange={(e) => handleInputChange('oPoNumber', e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange('oDeadlineAt', e.target.value)}
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => handleInputChange('oAddress', e.target.value)}
                    className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primaryColor"
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium mb-1">Catatan Pesanan</label>
                  <textarea
                    value={formData.oNotes}
                    onChange={(e) => handleInputChange('oNotes', e.target.value)}
                    className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primaryColor"
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-primaryColor">Item Pesanan</h2>
                <button
                  type="button"
              ...
              <div className="space-y-4">
                {formData.oItems.map((item, itemIndex) => (
                  <div key={itemIndex} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-lg text-primaryColor">Item #{itemIndex + 1}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        <label className="block text-sm font-medium mb-1">Catalogue Product *</label>
                        <select
                          value={item.cpId}
                          onChange={(e) => handleOrderItemChange(itemIndex, 'cpId', e.target.value)}
                          className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primaryColor"
                          required
                        >
                          ...
                          {Array.isArray(catalogueProducts) && catalogueProducts.map((product) => (
                            <option key={getId(product)} value={getId(product)}>
                              {getId(product)} - {getName(product)}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1">Inventory Stock *</label>
                        <select
                          value={item.isId}
                          onChange={(e) => handleOrderItemChange(itemIndex, 'isId', e.target.value)}
                          className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primaryColor"
                          required
                        >
                          ...
                          {(() => {
                            ...
                            const uniqueStocks = Array.isArray(inventoryStocks) ? inventoryStocks.reduce((acc, stock) => {
                              const stockName = getName(stock, ['isName', 'iCode', 'iDescription', 'name']);
                              const existingIndex = acc.findIndex(item => 
                                getName(item, ['isName', 'iCode', 'iDescription', 'name']) === stockName
                              );
                            ...
                            return uniqueStocks.map((stock) => (
                              <option key={getId(stock)} value={getId(stock)}>
                                {getName(stock, ['isName', 'iCode', 'iDescription', 'name'])}
                              </option>
                            ));
                          })()}
                        </select>
                                         ...
                                         {/* Mockup Images */}
                     <div className="mb-4">
                       <label className="block text-sm font-medium mb-2">Mockup Images</label>
                       <input
                         type="file"
                         id={`mockup-upload-${itemIndex}`}
                         multiple
                         accept="image/jpeg,image/jpg,image/png,image/webp"
                         onChange={(e) => handleMockupImageChange(itemIndex, e.target.files)}
                         className="hidden"
                       />
                       <label
                           ...
                           <div className="space-y-2">
                             {item.oiMockupImage.map((img, idx) => (
                               <div key={idx} className="flex items-center gap-4 bg-gray-50 p-2 rounded">
                                 <span className="font-semibold text-gray-700 truncate flex-1" title={img.name}>
                                   {img.name}
                                 </span>
                                 ...
                                 <button
                                   ...
                                   onClick={() => {
                                     ...
                                     handleOrderItemChange(itemIndex, 'oiMockupImage', newFiles);
                                     
                                     // Cleanup preview URL
                                     if (imagePreviewUrls[itemIndex] && imagePreviewUrls[itemIndex][idx]) {
                                       URL.revokeObjectURL(imagePreviewUrls[itemIndex][idx]);
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-gray-700">Ukuran dan Jumlah</h4>
                        <button
                          type="button"
                        ...
                        {item.oiSizes.map((size, sizeIndex) => (
                          <div key={sizeIndex} className="flex items-center gap-3">
                            <div className="flex-1">
                              ...
                              <select
                                value={size.sId}
                                onChange={(e) => handleSizeChange(itemIndex, sizeIndex, 'sId', e.target.value)}
                              ...
                              >
                                ...
                                {Array.isArray(sizes) && sizes.map((sizeOption) => (
                                  <option key={getId(sizeOption)} value={getId(sizeOption)}>
                                    {getName(sizeOption, ['sName', 'name'])}
                                  </option>
                                ))}
                              </select>
                            </div>
                            <div className="flex-1">
                              ...
                              <input
                                ...
                                onChange={(e) => handleSizeChange(itemIndex, sizeIndex, 'oisAmount', e.target.value)}
                                className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primaryColor text-sm"
            ...
            <div className="flex justify-end gap-4">
              ...
              >
                {uploading ? 'Mengupload gambar...' : saving ? 'Menyimpan...' : 'Buat Pesanan'}
              </button>
            </div>
          </form>
        </div>
