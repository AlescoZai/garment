...
import { ChevronDownIcon, ExclamationCircleIcon, PlusIcon, XMarkIcon, ShoppingCartIcon } from '@heroicons/react/24/outline';
...
import { getSizes, createOrder, uploadOrderMockupImage } from '../api/Order/order';
...
const OrderSection = ({ 
  ...
  }, [selectedProductId, selectedInventoryId, mockupFiles, selectedSizes, orderNumber, onOrderDataChange, catalogueProducts, inventories]);
  ...
  // Helper untuk mendapatkan ID dari berbagai format API
  const getId = (item, idFields = ['id', 'csId', 'ccId', 'cpId', 'iId', 'isId']) => {
  ...
  const getName = (item, nameFields = ['name', 'csName', 'ccName', 'cpName', 'iCode', 'iDescription', 'isName']) => {
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              <select 
                className="appearance-none w-full border-b border-gray-300 bg-transparent py-2 pr-8 text-sm focus:outline-none"
                ...
                onChange={(e) => {
                  ...
                  if (newProductId) {
                    ...
                    const product = catalogueProducts.find(p => getId(p, ['id', 'cpId']) === parseInt(newProductId));
              ...
              >
                ...
                {Array.isArray(catalogueProducts) && catalogueProducts.map((product) => (
                  <option key={getId(product, ['id', 'cpId'])} value={getId(product, ['id', 'cpId'])}>
                    {getId(product, ['id', 'cpId'])} - {getName(product, ['cpName', 'name'])}
                  </option>
                ))}
              </select>
            ...
            {selectedProductDetails && (
              <div className="bg-gray-50 border border-gray-200 p-3 rounded-md mt-2 text-sm">
                <p className="font-semibold text-gray-700 mb-1">Detail Produk:</p>
                <p className="text-gray-600">
                  <span className="font-medium">Nama Produk:</span> {getName(selectedProductDetails, ['cpName', 'name'])}
                </p>
                <p className="text-gray-600">
                  <span className="font-medium">Kategori:</span> {getName(selectedProductDetails, ['ccName', 'name'])}
                </p>
                <p className="text-gray-600">
                  <span className="font-medium">Sub Kategori:</span> {getName(selectedProductDetails, ['csName', 'name'])}
                </p>
              </div>
            )}
          </div>
              ...
              >
                <option value="">Pilih Bahan</option>
                  ...
                  const uniqueInventories = Array.isArray(inventories) ? inventories.reduce((acc, inventory) => {
                    const inventoryName = getName(inventory, ['isName', 'iCode', 'iDescription', 'name', 'description', 'code']);
                    const existingIndex = acc.findIndex(item => 
                      getName(item, ['isName', 'iCode', 'iDescription', 'name', 'description', 'code']) === inventoryName
                    );
                  ...
                  return uniqueInventories.map((inventory) => (
                    <option key={getId(inventory, ['id', 'iId', 'inventoryId'])} value={getId(inventory, ['id', 'iId', 'inventoryId'])}>
                      {getName(inventory, ['isName', 'iCode', 'iDescription', 'name', 'description', 'code'])}
                    </option>
                  ));
                })()}
              </select>
          ...
          <div className="flex flex-col items-center w-full">
            <label className="block text-gray-500 mb-2 text-center w-full font-semibold text-base">Please send us your design/ mockup</label>
            <input
              type="file"
              ...
              {mockupFiles.length === 0 ? (
                ...
                <div className="space-y-2 w-full max-w-full overflow-hidden">
                  {mockupFiles.map((file, index) => (
                    <div key={index} className="flex items-center text-base w-full justify-center gap-4 bg-gray-50 p-2 rounded overflow-hidden">
                      <span className="font-semibold text-gray-700 truncate max-w-xs" title={file.name}>
                        {truncateFileName(file.name)}
                      </span>
                      <span className="text-xs text-gray-500 flex-shrink-0">({Math.round(file.size / 1024)}KB)</span>
                      <button 
                        type="button" 
                        onClick={() => setMockupFiles(prev => prev.filter((_, i) => i !== index))}
...
export default function Order() {
  ...
  const extractData = useCallback((response) => {
    ...
    } else if (response.data.data && response.data.data.listData && Array.isArray(response.data.data.listData)) {
    ...
    const possibleArrayFields = ['list', 'records', 'rows', 'elements', 'collection'];
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setCatalogueProducts([
          { cpId: 1, cpName: 'Jaket Pharaoh', ccId: 1, ccName: 'Apparel', csId: 1, csName: 'Jaket' },
          { cpId: 2, cpName: 'Kemeja Lengan Panjang', ccId: 1, ccName: 'Apparel', csId: 2, csName: 'Kemeja' }
        ]);
      }
  ...
  const handleSubmitOrder = useCallback(async () => {
    // Validate form data
    const requiredFields = ['contactName', 'contactPhone', 'contactAddress', 'deadline'];
    ...
    for (const [orderNum, orderData] of Object.entries(orderItemsData)) {
      if (!orderData.cpId || !orderData.isId || orderData.oiSizes.length === 0) {
        invalidOrders.push(orderNum);
      }
    }
    
    if (invalidOrders.length > 0) {
      toast.error(`Order ${invalidOrders.join(', ')}: Mohon lengkapi produk, bahan, dan minimal satu ukuran!`);
      return;
    }
    ...
    try {
      ...
      const submissionData = {
        ...
        oDeadlineAt: formData.deadline ? new Date(formData.deadline).toISOString() : new Date().toISOString(),
      ...
      toast.update(loadingToastId, {
        render: 'Pesanan berhasil dikirim! Tim kami akan segera menghubungi Anda.',
        type: 'success',
        isLoading: false,
        autoClose: 3000
      });
      ...
      // Show error toast
      toast.update(loadingToastId, {
        render: error.response?.data?.message || 'Terjadi kesalahan saat mengirim pesanan. Silakan coba lagi.',
        type: 'error',
        isLoading: false,
        autoClose: 5000
      });
  ...
  return (
    <>
      <StickyNavbar />
      <div className={`flex flex-row items-start mb-16 bg-gray-100 min-h-screen relative transition-opacity duration-700 ${animatePage ? 'opacity-100' : 'opacity-0'}`}>
        {/* Left Section */}
        <div
          id="left-section"
          ref={leftSectionRef}
          style={{
            ...
            left: 0,
            height: leftSectionStyle.position === 'absolute' ? 'calc(100% - 5rem)' : undefined,
            width: '50%',
            zIndex: 10,
            transition: 'top 0.3s, position 0.3s',
            backgroundImage: 'url(https://images.unsplash.com/photo-1532453288672-3a27e9be9efd?q=80&w=764&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D)',
        ...
        >
          ...
          <div className="relative z-10 flex flex-row items-start w-full">
            ...
            <div className="flex flex-col justify-center">
              <h1 className="text-white text-5xl font-bold font-poppins leading-[1.1] mb-6 text-left">Jelasin<br />Kebutuhan<br />Kamu</h1>
              <p className="text-[#D9D9D9] text-base max-w-md font-montserrat text-left">Lorem ipsum dolor sit amet consectetur. Non sed commodo sed fermentum aliquam vulputate volutpat tortor hac.</p>
            </div>
          </div>
        </div>
          ...
          {dataLoading ? (
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-primaryColor font-semibold text-lg">Memuat data...</p>
                <p className="text-gray-500 text-sm mt-2">Mohon tunggu sebentar</p>
              </div>
            </div>
          ) : (
            <div className={`w-full max-w-xl space-y-6 font-montserrat transition-all duration-700 ${animatePage ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-red-700 font-semibold">
                      Terjadi beberapa error saat memuat data. Menggunakan data fallback.
                    </p>
                    <ul className="mt-2 text-xs text-red-600 list-disc list-inside">
                      {error.products && <li>Products: {error.products}</li>}
                      {error.subCategories && <li>Subcategories: {error.subCategories}</li>}
                      {error.inventories && <li>Inventories: {error.inventories}</li>}
            ...
            <div>
              ...
              <div className="relative flex items-center group">
                ...
                <div className="absolute right-0 top-8 bg-orange-50 border border-orange-200 rounded p-2 w-64 text-xs text-secondaryColor opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                  Nomor POS adalah nomor pemesanan atau referensi yang diberikan oleh sistem atau admin. Jika tidak tahu, silakan kosongkan.
                </div>
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Pilih Deadline</label>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <label className="block text-sm text-gray-500 mb-2">Any note you want to tell us?</label>
              <textarea
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
              ...
              {/* Alamat Pengiriman */}
              <div>
                <label className="block text-sm text-gray-500 mb-1">Alamat Pengiriman</label>
                <input
                  type="text"
